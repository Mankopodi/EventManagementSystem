  ...
  function Packages() {
      ...
      fetch(`${process.env.React_App_URl}/api/customize-events-packages`)
        ...
        .then(customizeEventsPackages => {
          ...
          console.log(customizeEventsPackages)
           console.log( 'getting user: ', customizeEventsPackages.data)
        })
    }
    ...
    return (
      <>
        ...
        <button className="btn btn-info" onClick={() => navigate("/dash/addpackage/", { replace: true })}>Add package</button>
              ...
              {customizeEventsPackages.map((customizeEventsPackage) => (
                <tr key={customizeEventsPackage.id}>
                  ...
                  <td style={{ color: 'black' }} >
                    <label for="my-modal-5" className="btn" onClick={() => navigate("/dash/EditUser/" + customizeEventsPackage.id, { replace: true })}> <FaEdit /> </label>
                  </td>

                  <td>
                    <label for="my-modal-5" className="btn" onClick={() => deletePackage(customizeEventsPackage.id)}>
